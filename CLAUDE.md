...
- **Error-response shape for API routes**: no JSON errors. Routes redirect back to the originating page with `?error=<message key>` (a key such as `errors.save.site` from `src/i18n`, never a sentence); the `.astro` page reads `Astro.url.searchParams.get("error")` and passes it to the React form as `serverError`, and the island (or the page, for server-rendered errors) translates it with `translateKey`, falling back to a generic message for anything unknown. Shared keys and `issueKey`/`authErrorKey` live in `@src/lib/api-errors.ts`. Follow this in new routes.
...
- **Coordinates never go into URLs or logs.** `?error=` values are catalogue keys from a fixed set, never form values or message parameters; `no-console` is an error under `src/lib/gear` and `src/pages/api/gear`. Gear routes redirect with a fixed `errors.save.*` key that deliberately hides the DB error, and auth routes map Supabase's `error.code` to a key (never forward `error.message`); to debug, reproduce against local Supabase, not by surfacing the error text.
- **Island imports**: browser islands import engine constants from `@/lib/engine/parameters` (the one exception to the barrel rule) and never import `src/lib/gear/timezone.ts` (server-only tz lookup) or `store.ts`. `src/i18n/**` is island-safe and must stay free of server-only imports; islands receive `locale` as a prop and call `getMessages(locale)` themselves.
