...
describe("searchPlaces", () => {
  it("requests up to five English results for the trimmed query, with a timeout", async () => {
    ...
    await searchPlaces("  Madrid ", "en", fake.fetchFn);
  ...
  it("asks for place names in the viewer's language", async () => {
    const fake = fakeFetch(() => jsonResponse({ results: [MADRID] }));
    await searchPlaces("Madryt", "pl", fake.fetchFn);
  ...
  it("returns [] when the response has no results key", async () => {
    ...
    await expect(searchPlaces("Xyzzy", "en", fake.fetchFn)).resolves.toEqual([]);
  });
  ...
  it("throws the fixed message when fetch rejects or times out", async () => {
    ...
    await expect(searchPlaces("Madrid", "en", network.fetchFn)).rejects.toThrow(new Error(GEOCODING_FAILED));
    const timeout = fakeFetch(() => Promise.reject(new DOMException("timed out", "TimeoutError")));
    await expect(searchPlaces("Madrid", "en", timeout.fetchFn)).rejects.toThrow(new Error(GEOCODING_FAILED));
  });
  ...
  it("throws the fixed message when the caller aborts", async () => {
    ...
    controller.abort();
    await expect(searchPlaces("Madrid", "en", fake.fetchFn, controller.signal)).rejects.toThrow(
      new Error(GEOCODING_FAILED),
    );
  ...
  it("never puts the query or URL into its error", async () => {
    ...
    const error = await searchPlaces("Secret Village", "en", fake.fetchFn).catch((e: unknown) => e);
    expect(error).toBeInstanceOf(Error);
    expect((error as Error).message).toBe(GEOCODING_FAILED);
    expect(translateKey(getMessages("en"), GEOCODING_FAILED, "errors.generic")).toBe(
