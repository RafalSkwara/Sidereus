...
/*
 * Deletes one eyepiece. RLS makes another user's eyepiece indistinguishable from a missing one, so both
...
export const POST: APIRoute = async (context) => {
  ...
  const result = await eyepieceStore.remove(supabase, context.params.id ?? "");
  if (!result.ok) {
